...
class VoidSettingsService extends Disposable implements IVoidSettingsService {
    ...
    // Read environment variable and set provider setting
		const voidAiProvider = process.env.VOID_AI_PROVIDER
		const voidAiApiKey = process.env.VOID_AI_API_KEY;
		if (voidAiProvider !== undefined && voidAiApiKey !== undefined) {
			this.setSettingOfProvider(voidAiProvider as any, 'apiKey' as any, voidAiApiKey);
		}
